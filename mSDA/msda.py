...
if __name__ == "__main__":
	...
	classification_data = np.load("classification_data.npy")
	classification_labels = np.load("classification_labels.npy")

	train_data, train_labels, test_data, test_labels = splitTrainTest(classification_data, classification_labels)
	...
	train_map, train_rep = mDA(train_data, prob_corruption, True, train_mapping)
	...
	clf_ordinary = svm.SVC().fit(train_data.T, train_labels)
	preds_ordinary = clf_ordinary.predict(test_data.T)
	ordinary_accuracy = np.mean(preds_ordinary == test_labels)
	print "Accuracy on regular data: ", ordinary_accuracy

	clf_deepRep = svm.SVC().fit(train_reps[-1].T, train_labels)
	preds_with_deepRep = clf_deepRep.predict(test_reps[-1].T)
	deep_accuracy = np.mean(preds_with_deepRep == test_labels)
	print "Accuracy with linear SVM on mSDA features: ", deep_accuracy

	clf_mcf = svm.SVC().fit(np.dot(train_data,train_map.transpose()).T, train_labels)
	preds_mcf = clf_mcf.predict(np.dot(test_data, train_map.transpose()).T )
